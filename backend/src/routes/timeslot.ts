...
router.post('/', requireLoggedIn, (req, res) => {
	...
	if (isNaN(start.getTime())) { errorResponse(res, new Error('Invalid start date'), 400); return; }
	if (isNaN(end.getTime()))   { errorResponse(res, new Error('Invalid end date'), 400);   return; }
	...
	const timeslot = new Timeslot({ start, end, canvasId, user: req.authorizedUser! });
	timeslot.save()
